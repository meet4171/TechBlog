...
export default function SignUp() {
    ...
    const formSchema = useMemo(() => (otpSent ? SignUpSchema : SignupEmailSchema), [otpSent]);
    ...
    const handleSendOtp = async (data: VerifySignupEmail) => {
        resetField('otp');
        setOtpSent(false);
        setTimeLeft(null);
        setOtpExpiry(null);
    ...
    const handleResendOtp = async () => {
        ...
        if (!email || !name || !agreeToTerms || resendCooldown) return;
        handleSendOtp({ email, name, agreeToTerms })
    ...
    const handleVerifyOtp = async (data: SignupData) => {
        try {
            setLoading(true);
    ...
    const submitHandler = (data: VerifySignupEmail | SignupData) => {

        if (!otpSent) {
            handleSendOtp(data as VerifySignupEmail)

        } else {
            handleVerifyOtp(data as SignupData);
        }
    }
        ...
        <div className="min-h-screen bg-background text-foreground flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                <div className="text-center">
                    <h2 className="mt-6 text-3xl font-bold">Join BlogSpace</h2>
